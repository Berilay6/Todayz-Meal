...
document.getElementById("logoutBtn").onclick = () => {
  //kullanıcı logout butonuna tıklayınca oturum kapatılır ve giriş sayfasına yönlendirilir.
  signOut(auth).then(() => (location.href = "login.html"));
};

document.getElementById("profileButton").onclick = () => {
  //profil butonuna tıklayınca profile.html sayfasına yönlendirilir.
...
async function fetchRecipes() {
  ...
  recipes.forEach(async (recipe) => {
    ...
    if ((vegan && !details.vegan) || (vegetarian && !details.vegetarian))
      //vegan-vejetaryen seçilmişse vegan-vejetaryen olmayan tarifler gösterilmez.
      return;
